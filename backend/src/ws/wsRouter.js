...
/**
 * Single WebSocketServer that routes connections by path.
 * ws does NOT support multiple WSS instances on the same http.Server natively —
 * using handleProtocols/path option only works for one instance.
 * Solution: one WSS with noServer:true + manual upgrade routing.
 */
export function setupWSRouter(server) {
  ...
  wss.on('connection', (clientWS, req) => {
    ...
    if (path === '/ws/ticks') {
      handleTicksClient(clientWS);
    } else if (path === '/ws/account') {
      handleAccountClient(clientWS);
    } else {
      clientWS.close();
    }
  });
...
function handleTicksClient(clientWS) {
  ...
  clientWS.on('message', (raw) => {
    try {
      ...
      console.error('[WSTicks] Message error:', e.message);
    }
  });

  clientWS.on('close', () => {
    console.log('[WSTicks] Client disconnected');
    remove();
  });
...
function handleAccountClient(clientWS) {
  ...
  clientWS.on('close', () => {
    console.log('[WSAccount] Client disconnected');
    remove();
  });
